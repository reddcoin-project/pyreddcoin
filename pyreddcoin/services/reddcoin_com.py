...
REDDCOIN_API_BASE_URL = "https://live.reddcoin.com"

from ..constants import SATOSHIS_PER_COIN

from .blockchain_client import BlockchainClient

class ReddcoinComClient(BlockchainClient):
...
def get_unspents(address, blockchain_client=ReddcoinComClient()):
    """ Get the spendable transaction outputs, also known as UTXOs or
        unspent transaction outputs.
    """
    if not isinstance(blockchain_client, ReddcoinComClient):
        raise Exception('A ReddcoinComClient object is required')
    ...
    try:
        ...
        raise Exception('Invalid response from reddcoin.com.')
    print unspents
    return format_unspents(unspents)

def broadcast_transaction(hex_tx, blockchain_client=ReddcoinComClient()):
    ...
    try:
        ...
        raise Exception('Received non-JSON from reddcoin.com.')
